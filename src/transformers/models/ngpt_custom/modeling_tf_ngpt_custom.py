...
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFAttention with GPT2->Ngpt_custom
...
@keras_serializable
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2MainLayer with GPT2->Ngpt_custom
class TFNgpt_customMainLayer(keras.layers.Layer):
...
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2PreTrainedModel with GPT2->Ngpt_custom
class TFNgpt_customPreTrainedModel(TFPreTrainedModel):
    """
...
@dataclass
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2DoubleHeadsModelOutput with GPT2->Ngpt_custom
class TFNgpt_customDoubleHeadsModelOutput(ModelOutput):
    """
...
class TFNgpt_customModel(TFNgpt_customPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFNgpt_customMainLayer(config, name="transformer")
...
class TFNgpt_customLMHeadModel(TFNgpt_customPreTrainedModel, TFCausalLanguageModelingLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFNgpt_customMainLayer(config, name="transformer")
...
class TFNgpt_customDoubleHeadsModel(TFNgpt_customPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        config.num_labels = 1
        self.transformer = TFNgpt_customMainLayer(config, name="transformer")
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(NGPT_CUSTOM_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=TFNgpt_customDoubleHeadsModelOutput, config_class=_CONFIG_FOR_DOC)
    def call(
        self,
        input_ids: TFModelInputType | None = None,
        past_key_values: Optional[Tuple[Tuple[Union[np.ndarray, tf.Tensor]]]] = None,
        ...
        return_dict: Optional[bool] = None,
        training: Optional[bool] = False,
    ) -> Union[TFNgpt_customDoubleHeadsModelOutput, Tuple[tf.Tensor]]:
        r"""
        ...
        ```python
        >>> import tensorflow as tf
        >>> from transformers import AutoTokenizer, TFNgpt_customDoubleHeadsModel

        >>> tokenizer = AutoTokenizer.from_pretrained("")
        >>> model = TFNgpt_customDoubleHeadsModel.from_pretrained("")
        ...
        return TFNgpt_customDoubleHeadsModelOutput(
            logits=lm_logits,
            mc_logits=mc_logits,
            past_key_values=transformer_outputs.past_key_values,
            hidden_states=all_hidden_states,
...
@add_start_docstrings(
    ...
    [`TFNgpt_customForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-1) do.
...
class TFNgpt_customForSequenceClassification(TFNgpt_customPreTrainedModel, TFSequenceClassificationLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
