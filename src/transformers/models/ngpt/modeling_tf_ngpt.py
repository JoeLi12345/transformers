...
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFAttention with GPT2->Ngpt
...
@keras_serializable
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2MainLayer with GPT2->Ngpt
class TFNgptMainLayer(keras.layers.Layer):
...
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2PreTrainedModel with GPT2->Ngpt
class TFNgptPreTrainedModel(TFPreTrainedModel):
    """
...
@dataclass
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2DoubleHeadsModelOutput with GPT2->Ngpt
class TFNgptDoubleHeadsModelOutput(ModelOutput):
    """
...
class TFNgptModel(TFNgptPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFNgptMainLayer(config, name="transformer")
...
class TFNgptLMHeadModel(TFNgptPreTrainedModel, TFCausalLanguageModelingLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFNgptMainLayer(config, name="transformer")
...
class TFNgptDoubleHeadsModel(TFNgptPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        config.num_labels = 1
        self.transformer = TFNgptMainLayer(config, name="transformer")
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(NGPT_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=TFNgptDoubleHeadsModelOutput, config_class=_CONFIG_FOR_DOC)
    def call(
        self,
        input_ids: TFModelInputType | None = None,
        past_key_values: Optional[Tuple[Tuple[Union[np.ndarray, tf.Tensor]]]] = None,
        ...
        return_dict: Optional[bool] = None,
        training: Optional[bool] = False,
    ) -> Union[TFNgptDoubleHeadsModelOutput, Tuple[tf.Tensor]]:
        r"""
        ...
        ```python
        >>> import tensorflow as tf
        >>> from transformers import AutoTokenizer, TFNgptDoubleHeadsModel

        >>> tokenizer = AutoTokenizer.from_pretrained("")
        >>> model = TFNgptDoubleHeadsModel.from_pretrained("")
        ...
        return TFNgptDoubleHeadsModelOutput(
            logits=lm_logits,
            mc_logits=mc_logits,
            past_key_values=transformer_outputs.past_key_values,
            hidden_states=all_hidden_states,
...
@add_start_docstrings(
    ...
    [`TFNgptForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-1) do.
...
class TFNgptForSequenceClassification(TFNgptPreTrainedModel, TFSequenceClassificationLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
