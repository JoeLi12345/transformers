...
from transformers import NgptConfig, is_tf_available
...
if is_tf_available():
    ...
    from transformers.models.ngpt.modeling_tf_ngpt import (
...
class TFNgptModelTester:
    ...
    def create_and_check_ngpt_model(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        model = TFNgptModel(config=config)
        inputs = {
            "input_ids": input_ids,
    ...
    def create_and_check_ngpt_lm_head(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        model = TFNgptLMHeadModel(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_ngpt_double_head(
        self, config, input_ids, input_mask, head_mask, token_type_ids, mc_token_ids, *args
    ):
        model = TFNgptDoubleHeadsModel(config=config)
    ...
    def create_and_check_ngpt_for_sequence_classification(
        ...
        model = TFNgptForSequenceClassification(config)
...
@require_tf
class TFNgptModelTest(TFModelTesterMixin, TFCoreModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (TFNgptModel, TFNgptLMHeadModel, TFNgptForSequenceClassification, TFNgptDoubleHeadsModel)
        if is_tf_available()
        else ()
    )
    all_generative_model_classes = (TFNgptLMHeadModel,) if is_tf_available() else ()
    test_head_masking = False
    test_onnx = True
    onnx_min_opset = 10

    def setUp(self):
        self.model_tester = TFNgptModelTester(self)
    ...
    def test_ngpt_lm_head(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_ngpt_lm_head(*config_and_inputs)

    def test_ngpt_double_head(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_ngpt_double_head(*config_and_inputs)
    ...
    def test_model_from_pretrained(self):
        model_name = ""
        model = TFNgptModel.from_pretrained(model_name)
    ...
    def test_onnx_runtime_optimize(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if model_class in [TFNgptForSequenceClassification, TFNgptDoubleHeadsModel]:
                continue
...
@require_tf
class TFNgptModelLanguageGenerationTest(unittest.TestCase):
    @slow
    def test_lm_generate_greedy_distilngpt_batch_special(self):
        model = TFNgptLMHeadModel.from_pretrained("distilbert/distilngpt")
        tokenizer = GPT2Tokenizer.from_pretrained("distilbert/distilngpt")
    ...
    @slow
    def test_lm_generate_sample_distilngpt_batch_special(self):
        model = TFNgptLMHeadModel.from_pretrained("distilbert/distilngpt")
        tokenizer = GPT2Tokenizer.from_pretrained("distilbert/distilngpt")
    ...
    @slow
    def test_lm_generate_greedy_distilngpt_beam_search_special(self):
        model = TFNgptLMHeadModel.from_pretrained("distilbert/distilngpt")
        tokenizer = GPT2Tokenizer.from_pretrained("distilbert/distilngpt")
    ...
    @slow
    def test_lm_generate_distilngpt_left_padding(self):
    ...
    @slow
    def test_lm_generate_ngpt_greedy_xla(self):
        model = TFNgptLMHeadModel.from_pretrained("")
    ...
    @slow
    def test_lm_generate_ngpt_sample_xla(self):
    ...
    @slow
    def test_lm_generate_ngpt_beam_search_xla(self):
        model = TFNgptLMHeadModel.from_pretrained("")
        tokenizer = GPT2Tokenizer.from_pretrained("")
    ...
    def test_contrastive_search_ngpt(self):
        ...
        ngpt_model = TFNgptLMHeadModel.from_pretrained("-large")
        input_ids = ngpt_tokenizer(article, return_tensors="tf")
    ...
    @slow
    def test_contrastive_search_ngpt_xla(self):
        article = (
            "DeepMind Technologies is a British artificial intelligence subsidiary of Alphabet Inc. and research "
            "laboratory founded in 2010. DeepMind was acquired by Google in 2014. The company is based"
        )
        ...
        xla_generate = tf.function(ngpt_model.generate, jit_compile=True)
